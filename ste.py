...
class Buffer:
    ...
    def undo(self, cu, window):
        i = ""
        count = 0
        f = open("temporary/log.txt")
        f2 = open("temporary/log.txt", "w")
        for i in f:
            pass
        self.insert(cu, i[7:])
        for i in range(len(i[7:])):
            window.right(cu, self)
            cu.right(self, window)
        f2.writelines(f.readlines()[:count])
        f.close()
        f2.close()
    ...
    def backspace(self, cu, buffer):
        if cu.col > 0:
            ...
            with open("temporary/log.txt", "a") as f:
                f.write("REMOVE " + current[cu.virtual_col - 1] + "\n")
                f.close()
...
def main(stdscr):
    ...
    with open("temporary/log.txt", "w") as f:
        pass
    key = ""
    h = True
    while True:
